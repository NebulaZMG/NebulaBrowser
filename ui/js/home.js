...
const SEARCH_ENGINE_KEY = 'nebula-search-engine';
let selectedSearchEngine = loadSelectedSearchEngine();

let bookmarks = [];

function loadSelectedSearchEngine() {
  try {
    const stored = localStorage.getItem(SEARCH_ENGINE_KEY);
    if (stored && Object.prototype.hasOwnProperty.call(searchEngines, stored)) {
      return stored;
    }
  } catch {}
  return 'google';
...
function applySelectedSearchEngine(engine) {
  if (!Object.prototype.hasOwnProperty.call(searchEngines, engine)) return;
  selectedSearchEngine = engine;
  const option = searchEngineDropdown?.querySelector(`[data-engine="${engine}"] img`);
  if (option && searchEngineLogo) {
    searchEngineLogo.src = option.getAttribute('src');
  }
  try {
...
async function loadBookmarks() {
  try {
    ...
    if (window.bookmarksAPI && typeof window.bookmarksAPI.load === 'function') {
      ...
      data = JSON.parse(localStorage.getItem('nebula-bookmarks') || '[]');
    }
...
function renderBookmarks() {
  ...
  bookmarks.forEach((b, index) => {
    ...
    box.onclick = () => requestNavigation(b.url);
...
searchEngineDropdown.addEventListener('click', (e) => {
  ...
  if (option) {
    applySelectedSearchEngine(option.dataset.engine);
    searchEngineDropdown.classList.add('hidden');
  }
});
// --- End Search Engine Dropdown Logic ---

applySelectedSearchEngine(selectedSearchEngine);
